...
class AppImageTarget extends (_core || _load_core()).Target {
    ...
    build(appOutDir, arch) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            yield copyDirUsingHardLinks(appOutDir, appInStageDir, true);
            const iconNames = yield (_bluebirdLst2 || _load_bluebirdLst2()).default.map(_this.helper.icons, function (it) {
                let filename = `icon-${it.size}.png`;
                if (it.file === _this.helper.maxIconPath) {
                    // largest icon as package icon
                    filename = `${_this.packager.executableName}.png`;
                }
                return (0, (_fs || _load_fs()).copyOrLinkFile)(it.file, _path.join(stageDir, filename), null, true).then(function () {
                    return { filename, size: it.size };
                });
            });
            ...
            if (arch === (_builderUtil || _load_builderUtil()).Arch.x64 || arch === (_builderUtil || _load_builderUtil()).Arch.ia32) {
                yield copyDirUsingHardLinks(_path.join(vendorDir, "lib", arch === (_builderUtil || _load_builderUtil()).Arch.x64 ? "x86_64-linux-gnu" : "i386-linux-gnu"), _path.join(stageDir, "usr/lib"), false);
            }
...
function copyDirUsingHardLinks(source, destination, useLink) {
